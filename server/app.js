...
io.on('connection', (socket) => {
	...
	socket.on('newPlayer', () => {
      ...
      var join_msg = "[ <font color='" + room.getPlayerColor(socket.handshake.address) + "'>Player " + room.getPlayerName(socket.handshake.address,socket) + " has joined!</font> ]<br>";
      io.sockets.emit("update messages", join_msg)
	});
	socket.on('disconnect', function() {
      log("User disconnected " + socket.handshake.address + ", " + socket.id)
      var leave_msg = "[ <font color='" + room.getPlayerColor(socket.handshake.address) + "'>Player " + room.getPlayerName(socket.handshake.address,socket) + " has left!</font> ]<br>";
    ...
    socket.on("send message", function(sent_msg, callback) {
        ...
        sent_msg = "[ <font color='" + room.getPlayerColor(socket.handshake.address) + "'>Player " + room.getPlayerName(socket.handshake.address,socket) + "</font> ]: " + sent_msg + "<br>";
